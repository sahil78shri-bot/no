...
const Reflection: React.FC = () => {
  ...
  const isSameWeek = (date1: string | Date, date2: Date) => {
    const d1 = typeof date1 === 'string' ? new Date(date1) : date1;
    return getWeekStart(d1).getTime() === getWeekStart(date2).getTime();
  };

  const updateReflectionField = (section: string, field: string, value: string) => {
    setCurrentReflection(prev => {
      const currentSection = prev[section as keyof WeeklyReflection];
      
      // Type-safe handling of nested objects
      if (typeof currentSection === 'object' && currentSection !== null && !Array.isArray(currentSection)) {
        ...
        // Initialize section if it doesn't exist or isn't an object
        return {
          ...prev,
          [section]: {
            [field]: value
  ...
  const saveReflection = async () => {
    const reflection: WeeklyReflection = {
      ...
      userId: 'mock-user', // TODO: Get from Auth0 user
    ...
    // TODO: Save to API
    setReflections((prev: WeeklyReflection[]) => [reflection, ...prev]);
  ...
  const renderStepContent = () => {
    ...
    const sectionData = (currentReflection[step.key as keyof WeeklyReflection] as Record<string, string>) || {};
          ...
          <div>
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => updateReflectionField('goals', 'progress', e.target.value)}
        ...
        {reflections.length > 0 && (
          <div className="card">
            ...
            {reflections.slice(0, 3).map(reflection => (
              ...
              >
                <h3>Week of {new Date(reflection.weekStartDate).toLocaleDateString()}</h3>
